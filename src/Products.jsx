...
import { fetchProducts, deleteProduct, editProduct, addProduct } from "./productsSlice";
...
export default function Products() {
  ...
  const [newProduct, setNewProduct] = useState({ name: "", category: "", stock: "", sold: "" });
  ...
  const handleSave = (id) => {
    dispatch(editProduct({ id, stock: Number(editValues.stock), sold: Number(editValues.sold) }));
    setEditRow(null);
  };
  ...
  if (status === "loading") return <p className="text-center mt-10">Loading products...</p>;

  // Filter & sort
  let displayedProducts = [...products];
  if (categoryFilter !== "All") displayedProducts = displayedProducts.filter((p) => p.category === categoryFilter);
  if (sortField === "stock") displayedProducts.sort((a, b) => b.stock - a.stock);
  ...
  return (
    <div className="p-6 space-y-6">
      {/* Header + Search + Sort */}
      <motion.div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4" variants={itemVariants} initial="hidden" animate="visible">
      ...
      {/* Add Product Form */}
      <motion.div className="flex flex-wrap gap-2 items-end" variants={itemVariants} initial="hidden" animate="visible">
        <input
          type="text"
          placeholder="Name"
      ...
      <motion.div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6" variants={containerVariants} initial="hidden" animate="visible">
        ...
        <motion.div className="bg-red-600 text-white p-4 rounded-lg shadow flex flex-col" variants={itemVariants}>
          ...
          <span className="text-2xl font-bold">{products.filter((p) => p.stock < 5).length}</span>
        </motion.div>
      </motion.div>
        ...
        <table className="min-w-full bg-white border rounded-lg shadow-md">
          ...
          <tbody>
            {displayedProducts.map((p) => (
              <motion.tr key={p.id} className="border-b hover:bg-gray-50" variants={itemVariants}>
                ...
                <td className="py-2 px-4">
                  {p.stock < 5 ? <span className="text-red-600 font-semibold">Low Stock</span> : <span className="text-green-600 font-semibold">In Stock</span>}
                </td>
                <td className="py-2 px-4 flex gap-2">
                  {editRow === p.id ? (
                    <button className="bg-green-500 text-white p-1 rounded hover:bg-green-600" onClick={() => handleSave(p.id)}>Save</button>
                  ) : (
                    <button className="bg-yellow-500 text-white p-1 rounded hover:bg-yellow-600" onClick={() => handleEdit(p)}>Edit</button>
                  )}
                  <button className="bg-red-500 text-white p-1 rounded hover:bg-red-600" onClick={() => handleDelete(p.id)}>Delete</button>
                </td>
              </motion.tr>
            ))}
          </tbody>
